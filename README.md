...
Theme-Vault-Marpは、Marpプレゼンテーション用の包括的なテーマコレクションと管理ツールを提供するリポジトリです。多様なデザインのテーマを集約し、ユーザーが簡単に利用、カスタマイズ、そして共有できる環境を提供します。バージョン: `v1.0.0`

## 🆕 最新情報

- 🎉 v1.0.0リリース: 初期テーマコレクション(`neko-kaisei-beige`テーマを含む)の追加、テーマ管理スクリプトの実装、ドキュメントの作成を行いました。

## ✨ 主な機能

1. 多様なテーマコレクション：ビジネス、学術、クリエイティブなど、様々な用途に適したテーマを提供 (現時点では`neko-kaisei-beige`テーマのみ)
2. 簡単なテーマ適用：Marpマークダウンファイルに1行追加するだけでテーマを適用可能
3. カスタマイズ支援：各テーマのCSSファイルを編集して、独自のスタイルを作成可能
4. テーマ管理ツール：Pythonスクリプトによるテーマの追加、更新、削除機能を提供
5. サンプルプレゼンテーション：`neko-kaisei-beige`テーマの使用例を示すサンプルスライドを同梱
...
### テーマのカスタマイズ:
1. `theme/[テーマ名]/[テーマ名].css`ファイルを開きます。
2. CSSを編集してスタイルをカスタマイズします。
...
### 🎨 テーマコレクション
- `theme/`ディレクトリ内に各テーマのCSSファイルとサンプルMarkdownファイルを格納
- 現在含まれるテーマ：`neko-kaisei-beige`

### 🛠 テーマ管理スクリプト
- `scripts/`ディレクトリ内にテーマの追加、更新、削除を行うPythonスクリプトを格納

### 📘 ドキュメント
- `README.md`に使用方法、カスタマイズガイド、貢献方法などを記載
...
- [Marp](https://marp.app/)チームに、素晴らしいプレゼンテーションツールを提供していただきありがとうございます。
- すべてのコントリビューターの皆様、ありがとうございます。
